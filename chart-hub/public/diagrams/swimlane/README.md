...
- Cross-team processes with clear owners
- SOP documentation for mixed handoffs
...
- `SwimlaneDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="swimlane"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 480">` containing every element of the
     diagram. `swimlane-title` / `swimlane-desc` provide accessible names.
...
- One lane per role
- Step nodes placed in their owner's lane
- Arrows crossing lanes at handoffs
- A focal step in accent

Lane crossings are the places where work gets stuck — make them visible.
...
```tsx
   ...
   export function Dashboard() {
     return <SwimlaneDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`swimlane-title`, patterns). Visual output is unaffected;
