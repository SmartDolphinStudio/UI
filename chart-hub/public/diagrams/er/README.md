...
- Product specs and schema discussions
- Onboarding to a domain's core entities
...
- `ErDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="er"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 480">` containing every element of the
     diagram. `er-title` / `er-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Entity boxes with a header and key fields
- Relationship lines labeled with verbs
- Cardinality hints at line ends
- A focal entity emphasized in accent

Keep it conceptual: the db-schema type is the physical counterpart.
...
```tsx
   ...
   export function Dashboard() {
     return <ErDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`er-title`, patterns). Visual output is unaffected;
