...
- Workflow and status-model specifications
- Backend state design docs
- Editorial and approval process explainers
...
- `StateMachine.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="state"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 460">` containing every element of the
     diagram. `state-title` / `state-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- State nodes with labels
- Transition arrows between states
- Rejection / revision loops drawn explicitly
- A focal transition in accent

Draw every allowed transition — the gaps are often the real spec.
...
```tsx
   ...
   export function Dashboard() {
     return <StateMachine className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`state-title`, patterns). Visual output is unaffected;
