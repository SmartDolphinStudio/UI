...
- Vendor and capability comparisons
- Skill or product profile assessments
- Multi-attribute scorecards
...
- `RadarChart.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="radar"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 500">` containing every element of the
     diagram. `radar-title` / `radar-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Spokes for each dimension with ring gridlines
- One polygon per entity
- A focal polygon in accent
- Legend and dimension labels

Best at 4-6 dimensions and 2-4 series — beyond that it turns to spaghetti.
...
```tsx
   ...
   export function Dashboard() {
     return <RadarChart className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`radar-title`, patterns). Visual output is unaffected;
