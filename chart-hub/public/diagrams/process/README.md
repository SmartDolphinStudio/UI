...
- Operations and SOP documentation
- Cross-team workflow explainers
- Handoff and ownership maps
...
- `ProcessDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="process"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1064 436">` containing every element of the
     diagram. `process-title` / `process-desc` provide accessible names.
  4. **Scoped CSS** — this diagram relies on CSS classes inside the
     SVG, so the component injects a small scoped stylesheet
     (`dd-process`-prefixed selectors) in the same effect. It cannot
...
- Owner lanes (customer, support, warehouse, finance)
- Step chips inside each lane
- Connectors with a focal step highlighted
- Legend for lane and step styling

The lanes answer 'who owns this step' before 'what happens next'.
...
```tsx
   ...
   export function Dashboard() {
     return <ProcessDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`process-title`, patterns). Visual output is unaffected;
