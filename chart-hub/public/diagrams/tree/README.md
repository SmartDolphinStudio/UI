...
- Taxonomies and skill catalogs
- Site maps and content structures
- Category-to-leaf hierarchy docs
...
- `TreeDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="tree"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 480">` containing every element of the
     diagram. `tree-title` / `tree-desc` provide accessible names.
...
- A root node
- Category branches at level two
- Leaf nodes under each category
- Connectors with a subtle curve

Three levels keep the tree legible at this density.
...
```tsx
   ...
   export function Dashboard() {
     return <TreeDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`tree-title`, patterns). Visual output is unaffected;
