...
- Product planning and release scoping
- Backlog workshops and roadmap framing
- MVP definition with a release-cut line
...
- `UserStoryMap.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="story-map"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1032 540">` containing every element of the
     diagram. `story-map-title` / `story-map-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Activity backbone across the top
- Step cards beneath each activity
- A release-cut line slicing releases 1-3
- The riskiest story flagged in accent

The release-cut line is the decision; the risk flag is the warning.
...
```tsx
   ...
   export function Dashboard() {
     return <UserStoryMap className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`story-map-title`, patterns). Visual output is unaffected;
