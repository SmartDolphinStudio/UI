...
- Budget and cost flow breakdowns
- Funnel and pipeline volume analysis
- Energy, traffic, or time allocation stories
...
- `SankeyDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="sankey"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 560">` containing every element of the
     diagram. `sankey-title` / `sankey-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- A single source band
- Stage bands whose width encodes volume
- Outcome bands where flows merge back
- One anomaly flow (flaky reruns) in accent

Flow width must equal the value; the accent marks the waste.
...
```tsx
   ...
   export function Dashboard() {
     return <SankeyDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`sankey-title`, patterns). Visual output is unaffected;
