...
- Performance and latency distribution comparisons
- Any 'two peaks vs. one peak' distribution story
- Multi-group density comparison
...
- `RidgelinePlot.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="ridgeline"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 500">` containing every element of the
     diagram. `ridgeline-title` / `ridgeline-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- One ridge path per service, stacked vertically
- A single shared amplitude scale
- Bin ticks and value labels
- The anomalous second peak highlighted in accent

Ridges share amplitude so shapes compare honestly — never normalize each ridge.
...
```tsx
   ...
   export function Dashboard() {
     return <RidgelinePlot className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`ridgeline-title`, patterns). Visual output is unaffected;
