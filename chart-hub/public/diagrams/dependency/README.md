...
- Package health reviews and refactor planning
...
- `DependencyGraph.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="dependency"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 600">` containing every element of the
     diagram. `dependency-title` / `dependency-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Packages ranked by fan-in (dependents)
- Directed edges between packages
- A back-edge cycle highlighted as the one anomaly
- Legend for rank and edge meaning

Rank the nodes before drawing: fan-in is the story.
...
```tsx
   ...
   export function Dashboard() {
     return <DependencyGraph className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`dependency-title`, patterns). Visual output is unaffected;
