...
> Ordered by cadence and leverage.

![Preview](preview.png)

## Overview

The **Pyramid Diagram** is a self-contained editorial diagram. Ordered by cadence and leverage. It ships as a **single-file React component** (inline SVG + Tailwind CSS) with no chart library, no data files, and no external stylesheets — copy the file in and it renders immediately.

Content pyramid showing short posts, essays, long-form guides, and a flagship book ordered by shipping cadence and leverage.

## When to use it

- Content strategy and funnel explainers
- Tiered model diagrams (volume vs. leverage)
- Top-of-funnel to bottom-of-funnel narratives
...
- `PyramidDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="pyramid"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 500">` containing every element of the
     diagram. `pyramid-title` / `pyramid-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Tiers stacked wide-to-narrow
- Each tier labeled with its cadence
- A focal tier in accent
- A caption explaining the ordering axis

The ordering axis (cadence x leverage) is the message, not the tiers.
...
```tsx
   ...
   export function Dashboard() {
     return <PyramidDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`pyramid-title`, patterns). Visual output is unaffected;
