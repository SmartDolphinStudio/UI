...
- Growth and KPI dashboards
- Cohort trends and funnel follow-through
- Any time-series with 2-4 series
...
- `LineChart.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="line"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 500">` containing every element of the
     diagram. `line-title` / `line-desc` provide accessible names.
...
- One line per series with point markers
- Series labels near the line ends
- A legend and axis captions

Keep series under four — after that the editorial style collapses.
...
```tsx
   ...
   export function Dashboard() {
     return <LineChart className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`line-title`, patterns). Visual output is unaffected;
