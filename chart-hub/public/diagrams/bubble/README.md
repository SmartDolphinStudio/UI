...
- Service health maps (latency x error rate x traffic)
- Portfolio or market positioning with a third sizing variable
- Risk matrices where bubble size adds severity
...
- `BubbleChart.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="bubble"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 500">` containing every element of the
     diagram. `bubble-title` / `bubble-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Zero-based axes with gridlines and tick labels
- Bubbles sized by a third variable, labeled
- A focal bubble highlighted in accent
- Axis captions that name the metrics

Bubble area must be proportional to the value — never radius.
...
```tsx
   ...
   export function Dashboard() {
     return <BubbleChart className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`bubble-title`, patterns). Visual output is unaffected;
