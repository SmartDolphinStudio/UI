...
- Config and instruction inheritance docs
- Folder and scope hierarchy explainers
...
- `NestedContainers.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="nested"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 500">` containing every element of the
     diagram. `nested-title` / `nested-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Outermost scope box, inner boxes nested inside
- Each level labeled with its scope name
- Inheritance arrows from broad to narrow
- A focal level in accent

Nesting depth past four gets hard to read; stop there.
...
```tsx
   import NestedContainers from "./components/NestedContainers";

   export function Dashboard() {
     return <NestedContainers className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`nested-title`, patterns). Visual output is unaffected;
