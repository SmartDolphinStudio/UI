...
- Hourly or seasonal demand cycles
- Any radial category data on a percentage scale
- Clock-time narratives (peak windows)
...
- `PolarChart.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="polar"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1000 520">` containing every element of the
     diagram. `polar-title` / `polar-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Concentric percentage rings
- Radial sector wedges per category
- The 100% peak sector in accent
- Radial tick labels around the rim

Works only when the radial form adds meaning — peaks around a clock.
...
```tsx
   ...
   export function Dashboard() {
     return <PolarChart className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`polar-title`, patterns). Visual output is unaffected;
