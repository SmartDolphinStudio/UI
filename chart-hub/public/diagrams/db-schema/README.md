...
- Migration planning and schema reviews
- Onboarding engineers to a codebase's data model
...
- `DbSchemaDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="db-schema"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="0 0 1080 560">` containing every element of the
     diagram. `db-schema-title` / `db-schema-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Table cards with a header and column rows
- SQL type chips per column and constraint chips
- Column-level foreign-key connectors between tables
- Schema group containers (public / billing)

The most data-dense diagram in the set; keep it for physical schemas, not concepts.
...
```tsx
   import DbSchemaDiagram from "./components/DbSchemaDiagram";

   export function Dashboard() {
     return <DbSchemaDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`db-schema-title`, patterns). Visual output is unaffected;
