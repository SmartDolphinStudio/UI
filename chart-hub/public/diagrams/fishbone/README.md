...
- Incident postmortems and RCA workshops
- Quality and reliability documentation
- Brainstorming sessions that need a shared frame
...
- `FishboneDiagram.tsx` — the whole diagram in one file:
  ...
  2. **Wrapper** — a `<div>` with Tailwind utilities (`w-full max-w-4xl mx-auto`)
     and a `data-diagram="fishbone"` attribute. Pass your own `className` to
     override sizing.
  3. **The SVG** — one `<svg viewBox="-40 0 1480 600">` containing every element of the
     diagram. `fishbone-title` / `fishbone-desc` provide accessible names.

## Visual layers

Reading the SVG top to bottom:

- Spine with the problem statement at the head
- Cause-category branches off the spine
- Candidate causes listed along each branch
- The confirmed root cause highlighted in accent

The accent marks the confirmed cause — one, not several.
...
```tsx
   ...
   export function Dashboard() {
     return <FishboneDiagram className="w-full max-w-4xl mx-auto" />;
   }
...
- **Duplicates** — rendering the same component twice on one page repeats
  internal SVG ids (`fishbone-title`, patterns). Visual output is unaffected;
